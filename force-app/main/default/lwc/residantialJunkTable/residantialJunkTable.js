...
export default class DealJunctionObjectTable extends NavigationMixin(LightningElement) {
    ...
    getobjectInfo(result) {
        if (result.data) {
            ...
            this.recordTypeId = Object.keys(rtis).find((rti) => rtis[rti].name === 'Residential');
	...
	@wire(getPicklistValues,
        {
            recordTypeId: '$recordTypeId',
            fieldApiName: Apartment_Type__FIELD
        }
    )
    LTFinfoValue6_5({error, data}) {
        if (data) {
			console.log('data.values '+data.values);
            let optionsValue = data.values;
			this.options6_5 = [{ label: '--None--', value: "" }];
			optionsValue.forEach(opt => this.options6_5.push(opt));
			console.log('this.options5 '+this.options6_5);
        } else if (error) {
            console.log(error);
        }
    }

	handleChange6_5(event) {
        this.value6_5 = event.detail.value;
		this.getData();
    }
